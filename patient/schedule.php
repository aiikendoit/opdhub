...
if (isset($_SESSION["user"])) {
    if ($_SESSION["user"] == "" or $_SESSION['usertype'] != 'p') {
        ...
        // Prepare and execute the statement to fetch user information
        ...
        if ($result->num_rows > 0) {
            $userrow = $result->fetch_assoc(); // Fetch the user data
            $userid = $userrow["pid"];
            $username = $userrow["pname"];
            // Further code handling the user data
        } else {
            // Handle the case when no user data is retrieved
            // Redirect, display an error message, or perform other necessary actions.
        }
    }
} else {
    header("location: ../login.php");
